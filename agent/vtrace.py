...
VTraceFromNormalDistributionReturns = collections.namedtuple(
    'VTraceFromNormalDistributionReturns',
    [
      'vs', 
      'pg_advantages', 
...
VTraceReturns = collections.namedtuple(
    'VTraceFromLogitsReturns',
    [
      'vs', 
      'pg_advantages'
...
def log_probs_from_logits_and_actions(policy_logits, actions):
  ...
  """
  policy_logits = tf.convert_to_tensor(
    policy_logits, 
    dtype=tf.float32
  )
...
def probs_from_normal_distribution(mus, sigmas, actions):
  """Computes action log-probs from policy logits and actions.
  ...
  Args:
    policy_logits: A float32 tensor of shape [T, B, NUM_ASSETS] with
      un-normalized log-probabilities parameterizing a softmax policy.
    actions: An int32 tensor of shape [T, B] with actions.
  ...
  mus = tf.convert_to_tensor(
    mus, 
    dtype=tf.float32
  )

  sigmas = tf.convert_to_tensor(
    sigmas, 
    dtype=tf.float32
  )
  ...
  mus.shape.assert_has_rank(3)
  sigmas.shape.assert_has_rank(3)
  ...
  return norm_dist.prob(actions)

def log_probs_from_normal_distribution(mus, sigmas, actions):
  """Computes action log-probs from policy logits and actions.
  ...
  return norm_dist.log_prob(actions)

def from_normal_distribution(
  behavior_policy_mus,
  behavior_policy_sigmas,
  target_policy_mus,
  target_policy_sigmas,
  actions,
  discounts,
  rewards,
  values,
  ...
  clip_rho_threshold=1.0, 
  clip_pg_rho_threshold=1.0,
  name='vtrace_from_normal_distribution'
):
  r"""V-trace for softmax policies.
  ...
  Returns:
    A `VTraceFromLogitsReturns` namedtuple with the following fields:
      ...
      target_action_log_probs: A float32 tensor of shape [T, B] containing
        target policy action probabilities (log \pi(a_t)).     
  """
  target_policy_mus = tf.convert_to_tensor(
    behavior_policy_mus, 
    dtype=tf.float32
  )

  target_policy_sigmas = tf.convert_to_tensor(
    behavior_policy_sigmas, 
    dtype=tf.float32
  )

  behavior_policy_mus = tf.convert_to_tensor(
    target_policy_mus, 
    dtype=tf.float32
  )

  behavior_policy_sigmas = tf.convert_to_tensor(
    target_policy_sigmas, 
    dtype=tf.float32
  )
  ...
  # Make sure tensor ranks are as expected.
  # The rest will be checked by from_action_log_probs.
  target_policy_mus.shape.assert_has_rank(3)
  target_policy_sigmas.shape.assert_has_rank(3)
  behavior_policy_mus.shape.assert_has_rank(3)
  behavior_policy_sigmas.shape.assert_has_rank(3)
  actions.shape.assert_has_rank(3)
  ...
  with tf.name_scope(
    ...
    # target policy with respect to a continuous action domain
    target_action_log_probs = log_probs_from_normal_distribution(
        mus=target_policy_mus,
        sigmas=target_policy_sigmas,
        actions=actions
    )

    # returns the sampling log probability of the actions for the
    # target policy with respect to a continuous action domain
    behaviour_action_log_probs = log_probs_from_normal_distribution(
        mus=behavior_policy_mus,
        sigmas=behavior_policy_sigmas, 
        actions=actions
    )

    # TODO check logical consistency
    # The difference between the sampling probability of the behavioral
    # policy and the target policy representing the log importance sampling 
    # weights, this gives a representation of the difference between thw
    # behavioral policy applied to a given state and the target policy
    # applied to that same state thus representing the extent of the 
    # drift of the behavioural policy from the target policy at any given
    # temporal state
    # Because the action space consists of multiple continuous actions that 
    # may or may not be mutually exclusive, the difference between the sampling
    # probability of all actions at a given time are summed representing the total
    # drift between the target policy and the behavioral policy at a given time
    actions_rank = actions.shape.ndims
    log_rhos = tf.reduce_sum(
    ...
    return VTraceFromNormalDistributionReturns(
        log_rhos=log_rhos,
        target_action_log_probs=target_action_log_probs,
        behaviour_action_log_probs=behaviour_action_log_probs,
        **vtrace_returns._asdict()
...
def from_importance_weights(
  ...
  """
  log_rhos = tf.convert_to_tensor(
    log_rhos, 
    dtype=tf.float32
  )

  discounts = tf.convert_to_tensor(
    discounts, 
    dtype=tf.float32
  )
  ...
  with tf.name_scope(
    ...
    # Defines the 
    delta_values = values_t_plus_1 - values
    discounted_rewards = rewards + discounts
    advantages = discounted_rewards * delta_values
    deltas = clipped_rhos * advantages

    # Note that all sequences are reversed, 
